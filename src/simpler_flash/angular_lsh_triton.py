...
class AngularLSHTriton(torch.nn.Module):
    ...
    def __init__(self, num_projs, dim, rng=None):
        ...
        if num_projs > 0:
            ...
            self.register_buffer(
                "proj_dir",
                torch.randn(dim + (num_projs,), generator=rng, dtype=torch.float16),
                persistent=False,
            )
            self.register_buffer(
                "enc_vec",
                2 ** torch.arange(self.num_projs, dtype=torch.float16).view(1, 1, 1, -1),
                persistent=False,
            )
        else:
    ...
    def _unit_hamming_distance_array(self, size_n):
        if size_n == 1:
            return torch.tensor([0, 1], dtype=torch.int16)
        a = self._unit_hamming_distance_array(size_n - 1)
        b = torch.cat([a, torch.flip(a, dims=[0]) + 2 ** (size_n - 1)], 0).to(torch.int16)
    ...
    def __repr__(self):
        return (
            f"AngularLSH(num_proj={self.num_projs}, proj_dir.shape={self.proj_dir.shape})"
        )
