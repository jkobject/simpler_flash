...
def prepare_chunk_indices(offsets: torch.LongTensor, chunk_size: int) -> torch.LongTensor:
    indices = torch.cat(
        [torch.arange(n) for n in triton.cdiv(prepare_lens(offsets), chunk_size).tolist()]
    )
...
def parallel_softpick_attn_fwd_kernel(
    ...
    for i_s in range(0, i_t * BT, BS):
        ...
        p_v = tl.make_block_ptr(
            v + (bos * H + i_h) * V, (T, V), (H * V, 1), (i_s, i_v * BV), (BS, BV), (1, 0)
        )
        # [BK, BS]
...
def parallel_softpick_attn_bwd_kernel_dq(
    ...
    for i_s in range(0, i_t * BT, BS):
        ...
        p_v = tl.make_block_ptr(
            v + (bos * H + i_h) * V, (V, T), (1, H * V), (i_v * BV, i_s), (BV, BS), (0, 1)
        )
        # [BK, BS]
        b_k = tl.load(p_k, boundary_check=(0, 1))
        # [BV, BS]
